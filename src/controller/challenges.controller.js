...
const selectTraining = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "SELECT id_ejercicios_ej,nombre_ejercicios,nombre_categoria,nombre_sub FROM retos_ejecicios, retos, categoria, sub_categoria, categoria_sub, ejercicios where id_retos_ej=id_retos and id_ejercicios_ej=id_ejercicios and id_categoria_cat=id_categoria and id_sub_cat= id_sub and categoria = id_sub and id_retos= $1",
      [id_retos_ej]
    );
...
const insertTraining = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "insert into retos_ejecicios (id_retos_ej, id_ejercicios_ej) values ($1,$2)",
      [id_retos_ej, id_ejercicios_ej]
    );
...
const deleteTraining = async (req, res) => {
  try {
    const {id_retos_ej, id_ejercicios_ej} = req.body;
    const response = await pool.query(
      "delete from retos_ejecicios where id_retos_ej = $1 and id_ejercicios_ej = $2",
      [id_retos_ej, id_ejercicios_ej]
    );
...
const cancelChallenges = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "delete from retos_ejecicios where id_retos_ej = $1",
      [id_retos_ej]
    );
    ...
    if (response.error) {
      ...
      const respuesta = await pool.query(
        "delete from retos where id_retos = $1",
        [id_retos]
      );
...
const selectAllChallenges = async (req, res) => {
  try {
    const response = await pool.query(
      "select * from retos, estado_reto where state=id_estado order by id_estado=1 asc"
    );
...
const updateStateChallenges = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "update retos set state = $1 where id_retos = $2",
      [state, id_retos]
    );
...
const selectClientRegistered = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "select documento, nombres, primer_apellido, segundo_apellido, numero_telefono, correo from usuarios, retos_usuarios, retos, telefono,  rol where id_retos_us=id_retos and documento_us=documento and documento_usuario=documento and rol=id_rol and rol=5 and id_retos = $1",
      [id_retos]
    );
...
const registeredClientToChallenges = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "insert into retos_usuarios (id_retos_us,documento_us) values ($1,$2)",
      [id_retos_us, documento_us]
    );
...
const searchDataChallenges = async (req, res) => {
  try {
    const {id_retos} = req.body;
    const response = await pool.query(
      "select * from retos where id_retos = $1",
      [id_retos]
    );
...
const updateChallengesNoImg = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "update retos set nombre_reto = $1, descripcion = $2, fecha = $3, state = $4, hora = $5  where id_retos = $6",
      [nombre_reto, descripcion, fecha, state, hora, id_retos]
    );
...
const selectDataFromChallenges = async (req, res) => {
  try {
    const {id_retos} = req.body;
    const response = await pool.query(
      "select id_retos, nombre_reto, descripcion, imagen, fecha, hora, estado_reto.estado from retos, estado_reto where state=id_estado and id_retos = $1",
      [id_retos]
    );
...
const selectTrainingFromChallenges = async (req, res) => {
  try {
    const {id_retos} = req.body;
    const response = await pool.query(
      "SELECT id_ejercicios, nombre_ejercicios, ejercicios.imagen, categoria, nombre_categoria, nombre_sub FROM ejercicios, categoria_sub, retos_ejecicios, retos, categoria, sub_categoria where categoria=id_sub and id_categoria_cat=id_categoria and id_sub_cat=id_sub and id_retos_ej=id_retos and id_ejercicios_ej=id_ejercicios and id_retos=$1",
      [id_retos]
    );
...
const selectStateChallengesUser = async (req, res) => {
  try {
    const response = await pool.query(
      "SELECT id_retos,nombre_reto, descripcion,imagen, concat(extract(day from fecha)||' '||'de'||' '|| to_char(fecha,'TMMonth')||' '||'del'||' '||extract(year from fecha)||'a las'||' '||extract(hour from hora)||':'||extract(minute from hora)),estado FROM retos,estado_reto where state=id_estado and state=1;"
    );
...
const validateUserAndChallenges = async (req, res) => {
  try {
    const {id_retos_us, documento_us} = req.body;
    const response = await pool.query(
      "select * from retos_usuarios where id_retos_us = $1 and documento_us = $2",
      [id_retos_us, documento_us]
    );
