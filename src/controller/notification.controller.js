...
const countRowsNotifications = async (req, res) => {
  try {
    const {documento} = req.body;
    const response = await pool.query(
      "select count(documento) as conteo from notificaciones,noti_usu, usuarios where id_noti_us=id_noti and documento_usu=documento and documento= $1",
      [documento]
    );
...
const deleteNotificationUser = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "delete from noti_usu where id_noti_us = $1 and documento_usu = $2",
...
const createNotification = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "insert into notificaciones (id_noti,texto) values ($1,$2)",
      [id_noti, texto]
    );
