...
router.post("/createChallenges", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const response = await pool.query(
      "insert into retos (id_retos, nombre_reto, descripcion, imagen, fecha, state, hora) values ($1,$2,$3,$4,$5,$6,$7)",
      [id_retos, nombre_reto, descripcion, imagen, fecha, state, hora]
    );
...
router.post("/updateChallengesImg", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const response = await pool.query(
      "update retos set nombre_reto = $1, descripcion = $2, imagen = $3, fecha = $4, state = $5, hora = $6  where id_retos = $7",
      [nombre_reto, descripcion, imagen, fecha, state, hora, id_retos]
    );
