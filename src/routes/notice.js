...
router.post("/createNotice", upload.single("file"), async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "insert into informacion (titulo,descripcion,autor,multimedia,fecha) values ($1,$2,$3,$4,$5)",
      [titulo, descripcion, autor, multimedia, fecha]
    );
...
router.post("/updateNoticeImg", upload.single("file"), async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "update informacion set titulo=$1, descripcion=$2, multimedia=$3 where id_informacion = $4",
      [titulo, descripcion, multimedia, id_informacion]
    );
